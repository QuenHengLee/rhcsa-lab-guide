...
一套為 **RHCSA (EX200, RHEL 10)** 準備的自我練習環境與雙語題解,涵蓋 38 個可自動評分的練習題,對照 Red Hat 官方考綱。適合在自己的 Rocky Linux / RHEL 10 虛擬機上反覆操作。

A self-contained practice environment and bilingual (English + 繁體中文) study guide for the **RHCSA (EX200, RHEL 10)** exam: 38 auto-graded exercises mapped to the official objectives, meant to be run over and over on your own Rocky Linux / RHEL 10 VM.
...
| 目錄 | 內容 |
|------|------|
| `guide/rhcsa-guide.html` | **題解手冊**:38 題的標準解法,每題四步驟(啟動 → 逐步解法 → 評分驗證 → 收尾),含考場心法與 man 查法。標 ★必考 #TAG 者為高機率考點 |
...
腳本會把 `lab` 指令、函式庫與 38 個練習裝到系統。裝好後,以任何有 sudo 權限的使用者：
...
essential-tools（find 依擁有者/名稱/權限/大小 · grep+tar · links）· users-and-groups(帳號 · nologin · login.defs 預設密碼期限)· permissions（ACL / setgid+sticky / umask）· storage（分割+UUID掛載 / LVM / LVM擴充 / Stratis / swap）· network-filesystems（rsync / NFS+autofs）· scheduling-and-services（at / cron / systemd timer / 服務控制 / tuned）· network-and-time（nmcli · chrony · hostname+/etc/hosts）· security（firewalld / SELinux boolean / SELinux 修復 / 非標準埠 httpd / SSH 金鑰）· containers（rootless podman · flatpak --user remote）· shell-scripting · processes-and-logs（nice / journal 持久化）· boot-and-recovery（emergency mode 修復 / **重設 root 密碼 init=/bin/bash**)
