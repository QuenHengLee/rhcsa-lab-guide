...
一套為 **RHCSA (EX200, RHEL 10)** 準備的自我練習環境與雙語題解,涵蓋 35 個可自動評分的練習題,對照 Red Hat 官方考綱。適合在自己的 Rocky Linux / RHEL 10 虛擬機上反覆操作。

A self-contained practice environment and bilingual (English + 繁體中文) study guide for the **RHCSA (EX200, RHEL 10)** exam: 35 auto-graded exercises mapped to the official objectives, meant to be run over and over on your own Rocky Linux / RHEL 10 VM.
...
| 目錄 | 內容 |
|------|------|
| `guide/rhcsa-guide.html` | **題解手冊**:35 題的標準解法,每題四步驟(啟動 → 逐步解法 → 評分驗證 → 收尾),含考場心法與 man 查法。標 ★必考 #TAG 者為高機率考點 |
...
| `labs/*.lab` | 35 個練習題定義(題目、環境佈置、自動評分、清理),中英雙語題目敘述 |
...
腳本會把 `lab` 指令、函式庫與 35 個練習裝到系統。裝好後,以任何有 sudo 權限的使用者：
